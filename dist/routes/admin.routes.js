...
const checkAdmin = async (req, res, next) => {
    try {
        ...
        const userResult = await database_1.pool.query('SELECT is_admin FROM users WHERE id = $1', [userId]);
...
router.get('/stats', async (req, res) => {
    ...
    `);
        ...
        const revenueChart = await database_1.pool.query(`
      SELECT 
    ...
    `);
        ...
        const topSpenders = await database_1.pool.query(`
      SELECT 
        u.id,
        u.username,
        u.telegram_id,
...
router.get('/users', async (req, res) => {
    ...
    `;
        ...
        const usersResult = await database_1.pool.query(query, params);
        const totalResult = await database_1.pool.query('SELECT COUNT(*) FROM users' + (search ? ' WHERE username ILIKE $1 OR telegram_id ILIKE $1' : ''), search ? [`%${search}%`] : []);
        res.json({
            success: true,
...
router.post('/users/:id/balance', async (req, res) => {
    try {
        ...
        try {
            const userResult = await database_1.pool.query('SELECT id, username FROM users WHERE id = $1', [id]);
           ...
           VALUES ($1, 'admin_adjustment', $2, $3)`, [id, balance, JSON.stringify({ reason, adminId: req.user.id, action: 'balance_adjustment' })]);
            }
            if (premium_balance !== undefined) {
                updates.push('premium_balance = premium_balance + $' + (params.length + 1));
                params.push(premium_balance);
                await database_1.pool.query(`INSERT INTO transactions (user_id, type, amount, metadata) 
           VALUES ($1, 'admin_adjustment', $2, $3)`, [id, premium_balance, JSON.stringify({ reason, adminId: req.user.id, action: 'premium_adjustment' })]);
            }
            params.push(id);
            await database_1.pool.query(`UPDATE users SET ${updates.join(', ')} WHERE id = $${params.length}`, params);
...
router.get('/cases', async (req, res) => {
    try {
        const casesResult = await database_1.pool.query(`
      SELECT 
        c.*,
...
router.post('/cases', async (req, res) => {
    try {
        const { id, name, type, price, premium_price, description, min_reward, max_reward, is_active } = req.body;
        if (!name || !type) {
            return res.status(400).json({ error: 'Название и тип обязательны' });
        }
        if (id) {
            // Редактирование
            const result = await database_1.pool.query(`UPDATE cases 
         SET name = $1, type = $2, price = $3, premium_price = $4, 
             description = $5, min_reward = $6, max_reward = $7, is_active = $8
         WHERE id = $9 
         RETURNING *`, [name, type, price, premium_price, description, min_reward, max_reward, is_active, id]);
            res.json({
                success: true,
                case: result.rows[0],
            ...
            // Создание
            const result = await database_1.pool.query(`INSERT INTO cases 
         (name, type, price, premium_price, description, min_reward, max_reward, is_active) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
         RETURNING *`, [name, type, price, premium_price, description, min_reward, max_reward, is_active]);
            res.json({
                success: true,
                case: result.rows[0],
                message: 'Кейс создан'
...
router.get('/skins', async (req, res) => {
    ...
    `;
        ...
        const skinsResult = await database_1.pool.query(query);
        // Реальные скины
        const realSkinsResult = await database_1.pool.query(`
      SELECT 
        rs.*,
...
router.get('/sponsors', async (req, res) => {
    try {
        const sponsorsResult = await database_1.pool.query(`
      SELECT 
        s.*,
...
router.post('/sponsors', async (req, res) => {
    try {
        const { id, name, username, invite_link, reward_type, reward_value, premium_reward, is_active, priority } = req.body;
        if (!name || !invite_link) {
            return res.status(400).json({ error: 'Название и ссылка обязательны' });
        }
        if (id) {
            // Редактирование
            const result = await database_1.pool.query(`UPDATE sponsors 
         SET name = $1, username = $2, invite_link = $3, reward_type = $4, 
             reward_value = $5, premium_reward = $6, is_active = $7, priority = $8
         WHERE id = $9 
         RETURNING *`, [name, username, invite_link, reward_type, reward_value,
            ...
            // Создание
            const result = await database_1.pool.query(`INSERT INTO sponsors 
...
router.get('/withdrawals', async (req, res) => {
    ...
    `;
        ...
        params.push(limit, offset);
        const withdrawalsResult = await database_1.pool.query(query, params);
        ...
        const totalResult = await database_1.pool.query(totalQuery, totalParams);
        res.json({
            success: true,
...
router.post('/withdrawals/:id/status', async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(`UPDATE withdrawal_requests 
       SET status = $1, admin_notes = $2, processed_at = CASE 
         WHEN $1 IN ('completed', 'rejected') THEN CURRENT_TIMESTAMP 
         ELSE processed_at 
       END
       WHERE id = $3 
       RETURNING *`, [status, admin_notes, id]);
...
router.get('/settings', async (req, res) => {
    try {
        const settingsResult = await database_1.pool.query('SELECT * FROM app_settings ORDER BY key ASC');
        res.json({
            success: true,
    ...
    try {
        ...
        try {
            for (const setting of settings) {
                await database_1.pool.query(`UPDATE app_settings 
           SET value = $1, updated_at = CURRENT_TIMESTAMP 
           WHERE key = $2`, [setting.value.toString(), setting.key]);
            }
            await database_1.pool.query('COMMIT');
            res.json({
                success: true,
...
router.get('/payments', async (req, res) => {
    ...
    `;
        ...
        params.push(limit, offset);
        const paymentsResult = await database_1.pool.query(query, params);
        const totalQuery = status !== 'all'
...
router.get('/export/:type', async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(query, [start, end]);
        // Преобразуем в CSV
        let csv = '';
        if (result.rows.length > 0) {
            // Заголовки

...
**Важно:** `build` использует только `vite build`, без `tsc`. Это избегает проблем с правами доступа на Vercel.
...
Для проверки типов перед деплоем используйте:

```bash
npm run type-check
```

Эта команда проверит типы, но не создаст файлы сборки.
...
**Ошибка:** `sh: line 1: /vercel/path0/node_modules/.bin/tsc: Permission denied`

**Решение:** Убедитесь, что `package.json` использует только `vite build`:

```json
{
  "scripts": {
    "build": "vite build"  // Не: "tsc && vite build"
  }
}
```

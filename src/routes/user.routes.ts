  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE users 
       ...
       WHERE id = $4 
       RETURNING id, telegram_id, username, first_name, last_name, avatar_url, balance`,
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const [userStats, casesStats, skinsStats, referralsStats] = await Promise.all([
      pool.query('SELECT total_earned, daily_streak, created_at FROM users WHERE id = $1', [userId]),
      pool.query('SELECT COUNT(*) FROM transactions WHERE user_id = $1 AND type = $2', [userId, 'case_open']),
      pool.query('SELECT COUNT(*) FROM inventory_items WHERE user_id = $1 AND is_fragment = false', [userId]),
    ...
    const marketResult = await pool.query(
      `SELECT 
       ...
       FROM transactions 
       WHERE user_id = $1 AND type LIKE 'market_%'`,
      [userId]
    );
...
router.post('/daily', async (req: Request, res: Response) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT balance, daily_streak, last_daily_at FROM users WHERE id = $1',
      [userId]
    );
    ...
    await pool.query(
      ...
      [userId, totalReward, JSON.stringify({ streak: newStreak, baseReward, streakBonus })]
    );

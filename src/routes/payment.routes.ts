...
router.post('/webhook/:provider', async (req: Request<{ provider: string }>, res: Response) => {
  try {
    ...
    if (status === 'succeeded') {
      // Находим платеж
      const paymentResult = await pool.query(
        'SELECT * FROM premium_payments WHERE payment_id = $1',
        [payment_id]
      );
...
router.get('/status/:paymentId', authenticate, async (req: Request<{ paymentId: string }>, res: Response) => {
  try {
    const userId = req.user!.id;
...
async function completePayment(paymentId: string, userId: string | number) {
  ...
  try {
    ...
    await client.query(
      `UPDATE users 
       ...
       WHERE id = $3`,
      [payment.amount_premium, payment.amount_rub, userId]
    );
